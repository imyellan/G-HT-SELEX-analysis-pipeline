...
workflow HTSELEX {
    ...
    Channel
        .fromPath(
            [
                "${params.ywuv_htselex}/*.fastq.gz",
                "${params.ywt_htselex}/*40N*_pTH*.fastq.gz",
                "${params.ywt_htselex}/*40N*_UT380*.fastq.gz",
                "${params.yww_htselex}/*.fastq.gz",
            ]
        )
        .map { fastq ->
    ...
    ywuv_for_catting = Channel
        ...
        .map { file ->
            ...
            def cycle = baseName.split('_')[1..2].join('_')
            // e.g., A_1
            def exp_id = baseName.split('_')[[0, 3..5]].join('_')
            // excluding the cycle and SXXX
    ...
    ywt_for_catting = Channel
        ...
        .map { file ->
            // Extract cycle (A_1, A_2, etc.) and unique identifier (e.g., pTH14647_TT40NCTCGTC_eGFP_IVT_SXXX)
            def baseName = file.getSimpleName()
            def cycle = baseName.split('_')[4]
            // e.g., A_1
            def exp_id = baseName.split('_')[0..3].join('_')
            // excluding the cycle and SXXX
    ...
    yww_for_catting = Channel
        ...
        .map { file ->
            // Extract cycle (A_1, A_2, etc.) and unique identifier (e.g., pTH14647_TT40NCTCGTC_eGFP_IVT_SXXX)
            def baseName = file.getSimpleName()
            def cycle = baseName.split('_')[1]
            // e.g., A_1
            def exp_id = baseName.split('_')[2..5].join('_')
            // excluding the cycle and SXXX
    ...
    ht_ght_individual = deduped_ht_selex
    //.mix(ght_individual)
...
workflow GHTSELEX {
    ...
    if ("${params.inputType}" == "fastq") {
        full_peaks = GHTPEAKCALLING(inputDir)
    }
...
workflow GHTPEAKCALLING {
    ...
    Channel
        ...
        .map { grouping_key, pair1, pair2 ->
            ...
            def cycle = baseName.split('_')[1].join('_')
            // e.g., A_1
            def exp_id = baseName.split('_')[[0, 2..6]].join('_')
            // excluding the cycle and SXXX
    ...
    ght_mapped_bam = ght_mapped_bams
        .filter { bam, baseName ->
    ...
    controls_collected = ght_control_bam_tuples.collect()

    // CALL PEAKS WITH MACS3
    // combined_bams = ght_mapped_bam_tuples.combine(ght_control_bam_tuples).view()
    ght_macse_peaks = PEAK_CALL(ght_mapped_bam_tuples, controls_collected)
...
workflow MOTIF_BENCHMARK_WORKFLOW {
    ...
    pfm_tuples = motif_dir_tuples
        .map { pfm_dir, base, kmerLen ->
...
process DE_DUP {
    ...
    """
        ...
        -cp ~/micromamba/opt/bbmap-39.08-0/current/ \
