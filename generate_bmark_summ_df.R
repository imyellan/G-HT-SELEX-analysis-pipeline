...
import_benchmark_summ <- function(bmark_path) {
  pTH <- 
    ifelse(grepl("PEAKS", bmark_path),
           basename(dirname(bmark_path)),
           ifelse(grepl("pTH", bmark_path),
                  gsub(".*_(pTH[0-9]+)_.*", "\\1", bmark_path),
                  gsub(".*_(UT[0-9]+-[0-9]+)_.*", "\\1", bmark_path)))
  ...
  # count number of "positives" included in the top scoring fraction
  bmark_pos_num <- as.numeric(readLines(file.path(dirname(bmark_path),
                                                  "pos_seq_count.txt")))
  bmark_pos_num <- ceiling(bmark_pos_num*as.numeric(bmark_pos_frac))
  cycle <- ifelse(
    grepl("merged_dimont", bmark_path), 
    "merged_dimont",
    ifelse(
      grepl("merged", bmark_path), "merged",
  ...
  bmark_df <- read_table(bmark_path,
    col_names = c("metric", "score"), progress = F,
    col_types = c("metric" = "c", "score" = "d")
  ) %>%
    mutate(
      pTH = pTH, motif = motif, cycle = cycle, Well = well, 
      selex_plate = selex_plate,
      bmark_pos_frac = bmark_pos_frac,
      bmark_pos_num = bmark_pos_num, exp_id = exp_id
    )
  bmark_df
}
...
if (reimport_bmarks) {
  ...
  plan(multicore)
  benchmark_summ_df <- benchmark_summ_fils %>%
    map(import_benchmark_summ) %>%
    list_rbind()
  plan(sequential)
